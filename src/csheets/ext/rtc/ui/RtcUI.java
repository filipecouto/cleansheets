 ...
 * a side bar and a CellDecorator (currently removed temporarily).
 * 
...
public class RtcUI extends UIExtension {
	...
	public JComponent getSideBar() {
		if (sidebar == null) {
			shareAction = new ShareAction((RealTimeCollaboration) getExtension(),
					uiController);
...
//	}
	...
	/**
	 * Tells the side bar to refresh the list of existing connections
	 */
	public void updateServersList(ArrayList<RtcCommunicator> communicators) {
		sidebar.updateServersList(communicators);
	}
	...
	public void onError(Object error) {
		if (error.equals("password")) {
			JOptionPane.showMessageDialog(sidebar, "Wrong Password!", "Password",
					JOptionPane.ERROR_MESSAGE);
		} else if (error.equals("refused")) {
			JOptionPane.showMessageDialog(sidebar,
					"Connection refused! Maybe the owner deactivated the share.",
