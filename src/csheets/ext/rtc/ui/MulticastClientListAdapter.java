 ...
 * 
 * @author Rita Nogueira; Gil Castro (gil_1110484)
 * 
 */
@SuppressWarnings("rawtypes")
public class MulticastClientListAdapter implements ListModel {
	...
	private boolean existInArray(ServerInformation shareInfo) {
		int shareCount = information.size();
		for (int i = 0; i < shareCount; i++) {
			if (shareInfo.getPort() == information.get(i).getPort()
					&& shareInfo.getIp().equals(information.get(i).getIp())) {
				return true;
			}
		}
		return false;
