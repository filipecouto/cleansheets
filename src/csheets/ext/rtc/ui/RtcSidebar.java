...
/**
 * @author Gil Castro (gil_1110484); Rita Nogueira
 */
public class RtcSidebar extends JPanel {
	...
	public RtcSidebar(RealTimeCollaboration extension,
		...
		listPanel.add(new JScrollPane(serverList));

		serverList.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
					serverList.setSelectedIndex(serverList.locationToIndex(e
							.getPoint())); // Manual Select
					...
					if (shareAdapter.isOwner(serverList.getSelectedIndex())) {
						// Deactivate|Activate
						JMenuItem deactivateItem = new JMenuItem(
								shareAdapter.isActivated(serverList.getSelectedIndex()) ? "Deactivate"
										: "Activate");
						deactivateItem.addActionListener(new ActionListener() {

							@Override
							public void actionPerformed(ActionEvent e) {
								shareAdapter.setActivated(
										serverList.getSelectedIndex(), !shareAdapter
												.isActivated(serverList.getSelectedIndex()));
							}
						});
						menu.add(deactivateItem);

						// Read-Only|Writable
						JMenuItem isWritableItem = new JMenuItem(
								shareAdapter.isWritable(serverList.getSelectedIndex()) ? "Read-Only"
										: "Writable");
						isWritableItem.addActionListener(new ActionListener() {

							@Override
							public void actionPerformed(ActionEvent e) {
								shareAdapter.setWritable(serverList.getSelectedIndex(),
										!shareAdapter.isWritable(serverList
												.getSelectedIndex()));
							}
						});
					...
					Rectangle cellBounds = serverList.getCellBounds(
							serverList.getSelectedIndex(),
							serverList.getSelectedIndex());
					menu.show(serverList, 0,
		...
		gl_contentPanel.setVerticalGroup(gl_contentPanel.createParallelGroup(
				...
				gl_contentPanel
						...
						.addComponent(serverList, GroupLayout.PREFERRED_SIZE, 140,
