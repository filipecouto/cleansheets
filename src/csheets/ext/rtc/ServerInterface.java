...
public class ServerInterface implements RtcCommunicator {
	 ...
	 * creates a Client instance to communicate with it and set the connection up
	 * 
	 * @param client
	...
	private void onClientConnected(Socket client) {
		try {
			...
			if (activated) {
				...
				newClient.sendMessage(new RtcMessage(info.getAddress(),
						MessageTypes.error, "refused"));
				newClient.close();
			}
		} catch (IOException e) {
			e.printStackTrace();
