...
public class ServerInterface implements RtcCommunicator {
     ...
     *
     * @param spreadsheet the index of the requested spreadsheet
     * @param range the range of the cells to get
     ...
     * any filled in cell
     *
     * @param index the index of the requested spreadsheet
     ...
     * up
     *
     * @param client the socket that connects to the client
     */
    private void onClientConnected(Socket client) {
        try {
            ...
            if(activated){
                ...
                newClient.sendMessage(new RtcMessage(info.getAddress(), MessageTypes.error, "refused"));
                newClient.close();
            }
        } catch (IOException e) {
            e.printStackTrace();
