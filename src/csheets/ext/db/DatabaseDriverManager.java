...
public class DatabaseDriverManager {
	...
	private DatabaseDriverManager() {
		...
		InputStream stream = CleanSheets.class.getResourceAsStream("ext/db/res/" + DRIVERS_FILENAME);
		if (stream != null) {
			try {
				driversList.load(stream);
			} catch (IOException e) {
				System.err.println("Could not load available drivers from: " + DRIVERS_FILENAME);
			} finally {
				try {
		...
		for (Map.Entry<Object, Object> driver : driversList.entrySet()) {
			...
			if (classPath == null) load(className);
			// else
	...
	public void load(Class extensionClass) {
		try {
			DatabaseExportInterface driver = (DatabaseExportInterface) extensionClass.newInstance();
			drivers.add(driver);
