...
public class DatabaseExportDialog extends JDialog {
	...
	Thread exportThread = new Thread(new Runnable() {
		...
		if (exportController == null) {
		    ...
		    exportController.setPrimaryKeys(primaryKeys
		...
		try {
		    ...
		    listener.onDatabaseInteraction(
			    exportController.getDriver(),
			    exportController.getDatabase(),
			    cells[0][0].getAddress(),
			    cells[cells.length][cells[0].length].getAddress(),
			    exportController.getTableName(),
			    spreadsheetNumber(cells[0][0].getSpreadsheet()));
		}
		enableButtons(true);
		setVisible(false);
	    }
	...
	PrimaryKeysAdapter adapter = (PrimaryKeysAdapter) primaryKeys
		.getModel();
